...
func (c *ProtoCid) Unmarshal(data []byte) (err error) {
	...
	if err != nil {
		if strings.HasSuffix(err.Error(), varint.ErrUnderflow.Error()) {
			c.Cid = cid.Undef
			return nil
		}
	}
...
func NewPopulatedProtoPubKey(_ randyNet) *ProtoPubKey {
	_, k, _ := crypto.GenerateKeyPair(crypto.Ed25519, crypto.MinRsaKeyBits)
...
func NewPopulatedProtoPrivKey(_ randyNet) *ProtoPrivKey {
	k, _, _ := crypto.GenerateKeyPair(crypto.RSA, crypto.MinRsaKeyBits)
